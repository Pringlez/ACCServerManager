...
```
docker build --rm -t acc-server:1.8.18 .
```
...
```
docker run -d --name acc-server-1 --restart unless-stopped --expose=9231-9232 --net=host -v <path-to-instance-configs-on-host>:/home/accserver/host -t acc-server:1.8.18
```
